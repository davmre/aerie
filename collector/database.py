...
def get_conversation_root(tweet_id: str, db_path: Path = DEFAULT_DB_PATH) -> str:
    """
    Walk up the reply chain to find the conversation root.
    Returns the tweet_id of the root (which may be the input itself if not a reply).
    """
    current_id = tweet_id
    with transaction(db_path) as conn:
        for _ in range(100):  # Safety limit
            row = conn.execute(
                "SELECT reply_to_tweet_id FROM tweets WHERE id = ?", (current_id,)
            ).fetchone()
...
def get_conversation_tweets(
    root_id: str, mode_id: str, db_path: Path = DEFAULT_DB_PATH
) -> list[dict]:
    """
    Get all approved tweets in a conversation rooted at root_id.
    Uses a recursive CTE to find all descendants, then filters to approved ones.
    Returns tweets as dicts with all fields.
    """
    with transaction(db_path) as conn:
        # First check if root itself is approved (it should be included even if no replies)
        rows = conn.execute(
            """
            WITH RECURSIVE conversation AS (
                -- Base: the root tweet
                SELECT t.* FROM tweets t WHERE t.id = ?
                UNION ALL
                -- Recursive: all replies to tweets in conversation
                SELECT t.* FROM tweets t
                JOIN conversation c ON t.reply_to_tweet_id = c.id
            )
            SELECT c.* FROM conversation c
            JOIN mode_decisions md ON c.id = md.tweet_id
...
def _tweet_engagement(tweet: dict) -> int:
    """Calculate engagement score for a tweet (likes + retweets)."""
    return (tweet.get("like_count") or 0) + (tweet.get("retweet_count") or 0)


def find_best_chain(tweets: list[dict]) -> list[dict]:
    """
    Given a list of tweets in a conversation, find the "best" linear chain.
    Best = longest chain, with ties broken by highest total engagement.
    Returns the chain as a list from root to leaf.
    """
    if not tweets:
        return []

    # Build a map of id -> tweet and id -> children
    by_id: dict[str, dict] = {t["id"]: t for t in tweets}
    children: dict[str, list[str]] = {t["id"]: [] for t in tweets}

    # Find the root (tweet with no parent in our set)
    root_id = None
    for tweet in tweets:
        parent_id = tweet.get("reply_to_tweet_id")
        if parent_id and parent_id in by_id:
            children[parent_id].append(tweet["id"])
        else:
            # This tweet's parent is not in our set, so it's a root
            if root_id is None:
                root_id = tweet["id"]
            else:
                # Multiple roots - pick the one with lower reply_to (more "rooty")
                if not tweet.get("reply_to_tweet_id"):
                    root_id = tweet["id"]

    if root_id is None:
        # Shouldn't happen, but return first tweet as fallback
        return [tweets[0]] if tweets else []

    # DFS to find all paths from root, tracking (length, -engagement, path)
    def dfs(node_id: str) -> tuple[int, int, list[str]]:
        """Returns (chain_length, negative_engagement, path) for best chain from node."""
        tweet = by_id[node_id]
        ...
        if not child_ids:
            # Leaf node
            return (1, -engagement, [node_id])

        # Find best child path
        best = (0, 0, [])
        for child_id in child_ids:
            child_result = dfs(child_id)
            # Compare by (length DESC, engagement DESC)
            # We use negative engagement so larger engagement sorts first
            if child_result[0] > best[0] or (
                child_result[0] == best[0] and child_result[1] < best[1]
            ):
                best = child_result

        # Extend with current node
        return (best[0] + 1, best[1] - engagement, [node_id] + best[2])

    _, _, path = dfs(root_id)
    return [by_id[tid] for tid in path]


def build_conversation_cluster(
    root_id: str, mode_id: str, db_path: Path = DEFAULT_DB_PATH
) -> dict[str, Any]:
    """
    Build a conversation cluster for display.
    Returns:
    {
        "root_id": str,
        "primary_chain": [tweet, ...],  # The best chain from root to leaf
        "hidden_reply_counts": {tweet_id: count, ...},  # Count of hidden replies per chain tweet
        "all_tweets": [tweet, ...],  # All approved tweets in conversation (for modal)
    }
    """
    tweets = get_conversation_tweets(root_id, mode_id, db_path)
    if not tweets:
        return {
            "root_id": root_id,
            "primary_chain": [],
    ...
    primary_chain = find_best_chain(tweets)
    chain_ids = {t["id"] for t in primary_chain}

    # Count hidden replies for each tweet in the chain
    hidden_reply_counts: dict[str, int] = {}
    for tweet in tweets:
        parent_id = tweet.get("reply_to_tweet_id")
        if parent_id and parent_id in chain_ids and tweet["id"] not in chain_ids:
            hidden_reply_counts[parent_id] = hidden_reply_counts.get(parent_id, 0) + 1
...
def get_direct_replies(
    tweet_id: str, mode_id: str, db_path: Path = DEFAULT_DB_PATH
) -> list[dict]:
    """
    Get all approved direct replies to a specific tweet.
    Used for the reply modal.
    """
    with transaction(db_path) as conn:
        rows = conn.execute(
            """
